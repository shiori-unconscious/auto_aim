...
namespace post_process
{
  ...
  {
    // 相机畸变矩阵与内参矩阵初始化
    std::string camera_info_path = "/home/ros/Desktop/user/ros2_ws/src/auto_aim_exec/config/mindvision_camera/camera_info.yaml";
    camera_info_init(camera_info_path);
    // 装甲板坐标系初始化
    armor_world_init(objP_small, objP_big);
    // 读取标签名称
    std::string label_path = "/home/ros/Desktop/user/onnx_to_engine/robomaster.names";
  ...
  void PostProcessNode::pub_angle_data(const std::vector<cv::Point2f> &target)
  {
    ...
    // {
    //   std::lock_guard<std::mutex> read_imu_lock(imu_mutex);
    //   Eigen::Quaterionf temp(q);
    //   q_ = temp;
    // }
    ...
    //  false: use solve posture angle to stm32
    // output_msg.angle_or_pix = true;
    // output_msg.class_num = output_box[target_box_id].cls;
    // // 负头偏右，应该往左转
    // // output_msg.angle_yaw = yawerr;
    // // // 负头往下偏，应该向上转
    // output_msg.angle_pitch = theta_x;
    // // output_msg.tvecs = tvecs;
    // output_msg.angle_yaw = theta_y;
    // output_msg.distance = sqrt(P.at<double>(0, 0) * P.at<double>(0, 0) + P.at<double>(0, 1) * P.at<double>(0, 1) + P.at<double>(0, 2) * P.at<double>(0, 2));
    // output_pub->publish(output_msg);
  }
