...
namespace target_detect
{
  TargetDetectNode::TargetDetectNode(rclcpp::NodeOptions const& options) : Node("target_detect", options)
  {
    ...
    cudaMalloc((void **)&image_float_buf, inter_buf_size * sizeof(float_t));
    cudaMalloc((void **)&input_buf, inter_buf_size * sizeof(float_t));
  ...
  void TargetDetectNode::do_obj_inference()
  {
    ...
    cudaMemcpy(obj_buffers[obj_input_idx], this->input_buf,
               obj_input_size * sizeof(float_t), cudaMemcpyDeviceToDevice);
    obj_context->executeV2(obj_buffers);

    output_msg->na = obj_output_dims.d[1];
    output_msg->no = obj_output_dims.d[2];
    
    cudaMemcpy(output_msg_temp.output.data(), obj_buffers[obj_output_idx], obj_output_size * sizeof(float_t), cudaMemcpyDeviceToHost);
  ...
  void TargetDetectNode::Pre_Process(int dst_height, int dst_width)
  {
    ...
    nppiConvert_8u32f_C3R(image_resize_buf, dst_width * 3,
                          image_float_buf, dst_width * 3 * sizeof(float_t), dstSize);
    resize_buf_lk.unlock();

    nppiMulC_32f_C3IR(input_scale, image_float_buf, dst_width * 3 * sizeof(float_t), dstSize);
    nppiSwapChannels_32f_C3IR(image_float_buf, dst_width * 3 * sizeof(float_t), dstSize, this->DstOrder);

    std::lock_guard<std::mutex> input_buf_lk(this->input_buf_mutex);
    float_t *r_plane = (float_t *)(this->input_buf);
    float_t *g_plane = (float_t *)(this->input_buf + dst_width * dst_height);
    float_t *b_plane = (float_t *)(this->input_buf + dst_width * dst_height * 2);
    float_t *dst_planes[3] = {r_plane, g_plane, b_plane};
    nppiCopy_32f_C3P3R(image_float_buf, dst_width * 3 * sizeof(float_t),
                       dst_planes, dst_width * sizeof(float_t), dstSize);
  }
}
